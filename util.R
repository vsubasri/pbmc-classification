...
# ----------------------
load_data <- function(dir,id,genes,size) {
        feats <- readRDS(file.path(dir,paste0(id,genes,'.rds')))
        ## training data
        train<-readRDS(file.path(dir,paste(id,genes,size,'train.rds', collapse="_")))
        x_train <- as.matrix(train[[1]])
        y_train <- factor(train[[2]])
        y_train <- factor(str_replace_all(train[[2]], "[^[:alnum:]]", ""))
        ## test data
        test<-readRDS(file.path(dir,paste(id,genes,'test.rds',collapse="_")))
        x_test <- as.matrix(test[[1]])
        y_test <- factor(test[[2]])
        y_test <- factor(str_replace_all(test[[2]], "[^[:alnum:]]", ""))

        colnames(x_train) <- colnames(x_test) <- paste("FEAT_",feats)
